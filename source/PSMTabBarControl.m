...
- (void)_animateCells:(NSTimer *)timer
{
    ...
    if ((cellCount > 0) && [animation isAnimating]) {
		...
		for (i = 0; i < [targetFrames count] && i < cellCount; i++) {
			currentCell = [_cells objectAtIndex:i];
...
- (void)mouseDragged:(NSEvent *)theEvent
{
    ...
    if (cell) {
		...
		if (_canDragTabs && distance >= 10 && !_didDrag && ![[PSMTabDragAssistant sharedDragAssistant] isDragging] &&
...
- (void)encodeWithCoder:(NSCoder *)aCoder 
{
    ...
    if ([aCoder allowsKeyedCoding]) {
		...
		[aCoder encodeBool:_canDragTabs forKey: @"PSMcanDragTabs"];
    }
}
